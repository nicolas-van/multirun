...
* *You need to deploy an old-school Unix-style multi-process "I've been coded in the 80s" (also known as "Threads are for noobs") service where the same base code yields multiple executable files that must all run concurrently:* Multirun can be useful.
